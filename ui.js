  ...
  html(params) {
    ...
    const groups = ATTR_GROUPS.map(g => `<div class="attr-group"><h4 class="subhead">${g.name}</h4><div class="attr-grid">${g.keys.map(k =>
      `<label class="attr-input"><span>${esc(ATTR_LABELS[k])}</span>
        <input type="range" min="1" max="100" value="${a[k]}" id="attr-${k}" oninput="this.parentNode.querySelector('output').textContent=this.value">
    ...
    return `<div class="view view--narrow">
      ...
      <section class="card">
        ...
        <div class="form-grid">
          ...
          <div class="field"><label>Hype</label><input id="f-hype" type="number" min="0" max="100" value="${b.hype}"></div>
          <div class="field"><label>Credibility</label><input id="f-cred" type="number" min="0" max="100" value="${b.credibility}"></div>
          <div class="field"><label>Popularity</label><input id="f-pop" type="number" min="0" max="100" value="${b.popularity}"></div>
      ...
      <details class="card card--details">
        ...
        <div class="form-grid">
          ...
          <div class="field"><label>Confidence</label><input id="h-conf" type="number" min="1" max="100" value="${h.confidence}"></div>
        </div>
    ...
    return `<div class="view view--narrow">
      <section class="card profile-hero">
        <div class="ph-top">
          ...
          <div class="ph-id">
            <h1>${esc(b.name)} ${b.status === 'retired' ? '<span class="chip chip--muted">Retired</span>' : ''}</h1>
      ...
      <section class="card"><h3>Attributes</h3>
        <p class="muted" style="margin-top:-4px">Every one of these feeds the fight engine. <button class="link-btn" id="pf-attrinfo">How attributes shape a fight →</button></p>
        ${attrs}
      </section>
