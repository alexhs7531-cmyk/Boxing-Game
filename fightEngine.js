...
function simulateFight(boxA, boxB, opts) {
  ...
  const fs = {
    a: { health: 100, stam: 100, cut: 0, fouls: 0 },
    b: { health: 100, stam: 100, cut: 0, fouls: 0 }
  };
  const totals = {
  ...
  for (let r = 1; r <= scheduled; r++) {
    ...
    cards.forEach(c => {
      const edge = (ro.scoreA + c.bias * 4) - ro.scoreB;
      let pa, pb;
      if (Math.abs(edge) < 2 && ro.kdA === 0 && ro.kdB === 0) { pa = 10; pb = 10; }
      else if (edge >= 0) { pa = 10; pb = 9; } else { pa = 9; pb = 10; }
      pb -= ro.kdB; pa -= ro.kdA; pa -= ro.foulA; pb -= ro.foulB;
      c.a += clamp(pa, 6, 10); c.b += clamp(pb, 6, 10);
    });

    const win = ro.scoreA > ro.scoreB + 1 ? 'A' : ro.scoreB > ro.scoreA + 1 ? 'B' : 'E';
  ...
  if (ended) {
    ...
    if (ended.winner === 'a') { winnerId = boxA.id; loserId = boxB.id; }
      ...
      const dec = decideDecision(cards, boxA.id, boxB.id);
...
function simulateRound(A, B, mAB, mBA, fs, r, scheduled) {
  ...
  fs.a.health = clamp(fs.a.health + (fs.a.health < 25 ? (2 + A.recovery * 2) : (3.8 + A.recovery * 4)), -25, 100);
  fs.b.health = clamp(fs.b.health + (fs.b.health < 25 ? (2 + B.recovery * 2) : (3.8 + B.recovery * 4)), -25, 100);
...
function knockdownRoll(att, def, power, counters, defState, m) {
  ...
  let p = (power + counters * 0.8) / 8 * (0.5 + att.powerRating) * (1.2 - def.chinRating) * healthOpen * 0.052 * m.koThreat;
  p = clamp(p, 0, 0.7);
...
function decideDecision(cards, aId, bId) {
  let ja = 0, jb = 0, jd = 0;
  ...
  if (jd >= 2) return { method: 'Draw', draw: true, winnerId: null, loserId: null };
  ...
  if (ta > tb) return { method: 'MD', winnerId: aId, loserId: bId, draw: false };
  if (tb > ta) return { method: 'MD', winnerId: bId, loserId: aId, draw: false };
...
function estimateFight(a, b, opts) {
  ...
  const wp = winProbability(a, b);
  const stakes = stakesScore(fake);

  const fav = wp.a >= wp.b ? a : b, dog = wp.a >= wp.b ? b : a;
  const favPow = (fav.attributes.power * 0.7 + fav.attributes.killerInstinct * 0.3) / 100;
  const dogChin = (dog.attributes.chin * 0.6 + dog.attributes.recovery * 0.2 + dog.hidden.durability * 0.2) / 100;
  const koThreat = (STYLE_PROFILES[a.style].koThreat + STYLE_PROFILES[b.style].koThreat) / 2;
  const stoppage = clamp(0.16 + favPow * 0.55 - dogChin * 0.35 + (koThreat - 1) * 0.45 + (rounds >= 12 ? 0.05 : 0), 0.04, 0.82);
  ...
  return {
    winA: Math.round(wp.a * 100), winB: Math.round(wp.b * 100),
    favId: fav.id,
