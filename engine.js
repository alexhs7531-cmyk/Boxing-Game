...
function applyAttributes(me, res, opp, fight) {
  ...
  const bump = (k, base) => {
    ...
    const nv = clamp(Math.round((me.attributes[k] + amt) * 100) / 100, 1, 100);
  ...
  if (won) {
    ...
    me.hidden.confidence = clamp(me.hidden.confidence + 3 + (ko ? 3 : 0), 1, 100);
    ...
    me.hidden.confidence = clamp(me.hidden.confidence - 4 - (ko ? 5 : 0), 1, 100);
    me.hidden.careerMomentum = clamp(me.hidden.careerMomentum - 7 - (ko ? 4 : 0), -100, 100);
    // bad performance erosion
    bump('stamina', -0.3);
    if (ko) { bump('chin', -0.8); bump('recovery', -0.4); me.hidden.durability = clamp(me.hidden.durability - 4, 1, 100); }
...
function ageBoxerOneYear(b) {
  ...
  const adj = (k, d) => { a[k] = clamp(Math.round((a[k] + d) * 10) / 10, 1, 100); };
  ...
  b.hidden.confidence = clamp(Math.round(b.hidden.confidence + (60 - b.hidden.confidence) * 0.1), 1, 100);
}
